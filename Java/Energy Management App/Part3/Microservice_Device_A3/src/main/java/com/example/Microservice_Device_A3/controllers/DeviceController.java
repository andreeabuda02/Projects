...
import com.example.Microservice_Device_A3.configurations.RabbitMQSender;
import com.example.Microservice_Device_A3.controllers.handlers.exceptions.ResourceNotFoundException;
import com.example.Microservice_Device_A3.dtos.DeviceDTO;
import com.example.Microservice_Device_A3.dtos.DeviceDataDTO;
import com.example.Microservice_Device_A3.dtos.DeviceDetailsDTO;
...
public class DeviceController {
    ...
    @Autowired
    public DeviceController(DeviceService deviceService, RabbitMQSender rabbitMQSender, JwtValidator jwtValidator) {
    ...
    public ResponseEntity<List<DeviceDTO>> getAllDevices(@RequestHeader HttpHeaders headers) {
        ...
        for (DeviceDTO d : list) {
            Link deviceLink = linkTo(methodOn(DeviceController.class).getDeviceById(d.getId_d(), new HttpHeaders())).withRel("device_details");
            d.add(deviceLink);
        }
    ...
    @GetMapping(value = "/{id}")
    public ResponseEntity<DeviceDetailsDTO> getDeviceById(@PathVariable("id") UUID id_d, @RequestHeader HttpHeaders headers) {
        if (!jwtValidator.validateToken(headers.getFirst("Authorization"))) {
            LOGGER.warn("Unauthorized access attempt for device ID: {}", id_d);
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
    ...
    @PostMapping()
    public ResponseEntity<UUID> addDevice(@Valid @RequestBody DeviceDetailsDTO deviceDetailsDTO, @RequestHeader HttpHeaders headers) {
        if (!jwtValidator.validateToken(headers.getFirst("Authorization"))) {
            LOGGER.warn("Unauthorized access attempt during device addition.");
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
    ...
    @PutMapping(value = "/{id}")
    public ResponseEntity<DeviceDetailsDTO> updateDevice(@Valid @RequestBody DeviceDetailsDTO deviceDetailsDTO, @PathVariable("id") UUID id_d, @RequestHeader HttpHeaders headers) {
        if (!jwtValidator.validateToken(headers.getFirst("Authorization"))) {
            LOGGER.warn("Unauthorized access attempt for device update.");
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
    ...
    public ResponseEntity<String> deleteDevice(@PathVariable("id") UUID id_d, @RequestHeader HttpHeaders headers) {
        if (!jwtValidator.validateToken(headers.getFirst("Authorization"))) {
            LOGGER.warn("Unauthorized deletion attempt for device ID: {}", id_d);
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
        LOGGER.info("Deleting device by ID: {}", id_d);
        try {
            String message = deviceService.deleteDevice(id_d);
    ...
    public ResponseEntity<List<DeviceDTO>> getDeviceByUserId(@PathVariable("id") UUID userId, @RequestHeader HttpHeaders headers) {
        if (!jwtValidator.validateToken(headers.getFirst("Authorization"))) {
            LOGGER.warn("Unauthorized attempt to fetch devices for user ID: {}", userId);
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
        ...
        for (DeviceDTO deviceDto : deviceDTOList) {
            Link deviceLink = linkTo(methodOn(DeviceController.class).getDeviceById(deviceDto.getId_d(), new HttpHeaders())).withRel("device_details");
