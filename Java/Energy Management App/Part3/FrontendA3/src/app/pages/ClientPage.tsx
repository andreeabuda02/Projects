...
import { USERS_HOST, DEVICES_HOST, MONITORING_HOST, WEBSOCKET_HOST, WEBSOCKET_MESSAGE_HOST, BROKER_MESSAGE_HOST, MESSAGING_HOST } from "./hosts";
...
export const ClientPage = (): JSX.Element => {
  ...
  const fetchEnergyUsageData = async () => {
    try {
      ...
      const response = await axios.get(
        ...
        selectedDeviceId +
        "/" +
        selectedDate, { headers: { 'Authorization': "Bearer " + location.state.token } }
      );
      setGraphData(
  ...
  const fetchUserDataAndDevices = async (email: string) => {
    try {
      const userResponse = await axios.get(
        USERS_HOST.backend_api + endpoint.user + email, { headers: { 'Authorization': "Bearer " + location.state.token } }
      );
      const userId = userResponse.data.id_u;
      setUserName(userResponse.data.name);

      const devicesResponse = await axios.get(
        DEVICES_HOST.backend_api + endpointd1.device + userId, { headers: { 'Authorization': "Bearer " + location.state.token } }
      );
  ...
  useEffect(() => {
    if (errorMessage) {
      const errorTimer = setTimeout(() => setErrorMessage(""), 5000);
  ...
  // Navigate to the ChatPage with the current user's data
    ...
    return (
      <div style={parentDivStyle}>
        <div style={buttonContainerStyle}>
          ...
          <Button style={chatButtonStyle} onClick={redirectToChatPage} variant="contained">
            Chat
          </Button>
        </div>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <Button variant="contained"onClick={() => setView("graph")}>
            Graph
          </Button>
        </div>
          ...
          <>
            ...
            <div>
              <FormControl fullWidth margin="normal">
                ...
                <Select labelId="device-select-label" value={selectedDeviceId} onChange={(e) => setSelectedDeviceId(e.target.value as string)}>
