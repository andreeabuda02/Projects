  ...
  const handleLogin = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        const userId = response.data.id_u;
        const response1 =await axios.post(USERS_HOST.backend_api + endpoint.user + "token_generation/" + userId);
        if(response1.status === 200){
          if (userRole === "admin") {
            navigate("/AdminPage", { state: { email:email, token:response1.data, userId:userId } });
          } else if (userRole === "client") {
            navigate("/ClientPage", { state: { email:email, token:response1.data, userId:userId } });
          } else {
